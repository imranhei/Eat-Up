...
export default Location = () => {
    ...
    const handleLocation = (place) => {
        setPlaceName(place)
        navigate('/restuarent', { state: { placeName } });
    };

    return (
        <>
            <h1 className="text-black font-extrabold md:text-5xl text-3xl text-center p-16">Select the place where you want to pick or deliver</h1>
            <div className="flex flex-wrap justify-center p-2 w-full gap-0 lg:gap-2 xl-gap-4">
                {
                    places.map((place, index) => (
                        <Link onClick={() => handleLocation(place)} to={`/${place}`} key={place} className="group lg:w-1/6 md:w-1/4 w-1/2 aspect-square relative p-2 lg:m-0 overflow-hidden cursor-pointer">
